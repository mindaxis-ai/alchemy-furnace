...
- **先理解，再回答**：语义层先判断用户真正想问什么，导演层再决定这次该说多长。随口一问通常只回一两句，需要分析或执行的任务才会展开。
- **保留人物声音**：回答生成后会经过 Humanizer 编辑，删去套话、重复总结、机械排比和客服式收尾，同时保留事实、数字、结论与人物本来的语气。
...
每轮对话先经过语义层。它结合当前问题和最近的对话，识别用户是在闲聊、发泄情绪、询问事实，还是需要建议、执行任务或深入分析。导演层根据这个结果给出确定的字数、句数、模型 token 和群聊发言人数预算。人物模型随后按自己的性格、能力、示例对白与当前预算写出草稿，Humanizer 再做一次只改表达的编辑。

普通单人对话一轮通常有三次模型调用：语义理解、人物草稿、Humanizer。开放式群聊还会由 Supervisor 多做一次发言规划；明确点名、全体发言和报数等指令沿用确定性路由，不需要这次规划。语义模型不可用时会回退到本地规则；Humanizer 失败或改坏了数字、网址、代码结构与长度时，系统会使用按预算收束后的原始草稿。

人物知道自己在炼丹炉里服用过哪些金丹。用户直接询问时，他会按记录如实回答；在普通对话里，金丹只是能力来源，不会让人物自称道人，也不会自动说成古风或修仙口吻。

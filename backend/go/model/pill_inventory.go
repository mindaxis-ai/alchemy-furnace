...
func (m *PillRecipe) BeforeCreate(tx *gorm.DB) error {
	if m.PillRecipeID == "" {
		m.PillRecipeID = uuid.New().String()
	}
	return nil
}
...
func (m *PillRecipeRevision) BeforeCreate(tx *gorm.DB) error {
	if m.PillRecipeRevisionID == "" {
		m.PillRecipeRevisionID = uuid.New().String()
	}
	return nil
}
...
type PillItem struct {
	...
	// 011 关联标签：关系列存父实体 UUID 文本，FK 按父表 uuid 列约束
	RecipeRevision   PillRecipeRevision `json:"-" gorm:"foreignKey:RecipeRevisionID;references:PillRecipeRevisionID;constraint:OnDelete:Restrict"`
	OriginOperation  PillOperation      `json:"-" gorm:"foreignKey:OriginOperationID;references:PillOperationID;constraint:OnDelete:Restrict"`
	ConsumeOperation *PillOperation     `json:"-" gorm:"foreignKey:ConsumeOperationID;references:PillOperationID;constraint:OnDelete:Restrict"`
}
...
type AgentPillEffect struct {
	...
	// 011 关联标签：关系列存父实体 UUID 文本，FK 按父表 uuid 列约束
	Item           PillItem           `json:"-" gorm:"foreignKey:ItemID;references:PillItemID;constraint:OnDelete:Restrict"`
	RecipeRevision PillRecipeRevision `json:"-" gorm:"foreignKey:RecipeRevisionID;references:PillRecipeRevisionID;constraint:OnDelete:Restrict"`
}
...
func (m *AgentPillEffect) BeforeCreate(tx *gorm.DB) error {
	if m.AgentPillEffectID == "" {
		m.AgentPillEffectID = uuid.New().String()
	}
	return nil
}
...
func (m *PillOperation) BeforeCreate(tx *gorm.DB) error {
	if m.PillOperationID == "" {
		m.PillOperationID = uuid.New().String()
	}
	return nil
}
...
type FusionPreview struct {
	...
	// 011 关联标签：关系列存父实体 UUID 文本，FK 按父表 uuid 列约束
	ConfirmedOperation *PillOperation `json:"-" gorm:"foreignKey:ConfirmedOperationID;references:PillOperationID;constraint:OnDelete:Restrict"`
}
...
func (m *FusionPreview) BeforeCreate(tx *gorm.DB) error {
	if m.FusionPreviewID == "" {
		m.FusionPreviewID = uuid.New().String()
	}
	return nil
}
...
func (m *PillStarterGrant) BeforeCreate(tx *gorm.DB) error {
	if m.PillStarterGrantID == "" {
		m.PillStarterGrantID = uuid.New().String()
	}
	return nil
}

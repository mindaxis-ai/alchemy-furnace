...
- dispatch_daoists 是一个原子节点：按计划顺序逐发言人串行子运行
  DaoistGraph（每发言人一张干净的瞬态切片），单道人失败只跳过该发言人，
  不阻塞剩余计划成员（设计文档 §10）；事件经共享 event_sink 流出，
  顺序 = 发言顺序。发言人粒度的中断/恢复由 ConversationGraph（后续 Task）
  在计划与已完成回复的差集上裁决。
- convergence 收束本回合：清空待发言者并标记 completed。
...
def _route_after_router(
    ...
    """条件边：节点只落计划，路由读单一决策（directive.kind + 计划有无）。"""
...
def convergence(state: ConversationState) -> dict[str, Any]:
    """收束本回合：计划内发言人已全部调度完，清空待发言者并标记完成。"""

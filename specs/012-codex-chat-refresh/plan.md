...
**Goal:** 保留原有炼丹炉配色，完成 Codex 风格聊天交互、输入框模型切换、金丹/人设修复、清理与桌面验证。
**Architecture:** 沿用 React ChatContext → Go ConversationCommand → Python 编排链路，模型覆盖限定当前会话发送请求；保留已有 API 默认行为与全局视觉令牌，聊天组件局部重排。
